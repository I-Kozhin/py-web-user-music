...
from app.models.audio_models import Audio, AudioDto
...
class AudioService:
    ...
    async def add_audio_wav(self, user_id: int, user_token: str, audio_wav, session: AsyncSession) -> AudioDto:
        if await self.is_valid_token_to_id(user_id, user_token, session):
            ...
            result = AudioDto.from_bd(result)
        else:
            pass

        return result

    async def get_audio_mp3(self, audio_id: str, user_id: int, session: AsyncSession) -> AudioDto:
